...
export function ContribGraph({ target = 1397 }: { target?: number }) {
  ...
  return (
    <section id="contrib" style={{ padding: mobile ? '48px 0 40px' : '80px 0 60px', borderTop: `1px solid ${T.rule}` }}>
      <Wrap>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: mobile ? 24 : 32, flexWrap: 'wrap', gap: 20 }}>
          <div>
            <div style={{ fontFamily: T.mono, fontSize: 11, color: T.muted, letterSpacing: '0.2em', textTransform: 'uppercase' }}>
              Signal in public
            </div>
            <h2 style={{ fontSize: mobile ? 32 : 44, letterSpacing: -1, fontWeight: 500, margin: '12px 0 0', lineHeight: 1.05 }}>
              <span style={{ color: T.accent }}>{target.toLocaleString()} contributions</span> in the last year.
            </h2>
            <p style={{ fontSize: mobile ? 14 : 15, color: T.subdued, margin: '12px 0 0', maxWidth: 640, lineHeight: 1.6 }}>
        ...
        <div style={{ padding: mobile ? 16 : 24, border: `1px solid ${T.rule}`, position: 'relative', overflow: 'auto' }}>
          <div style={{ display: 'flex', marginLeft: 32, gap: 0, height: 16, fontFamily: T.mono, fontSize: 10, color: T.muted, letterSpacing: '0.1em', minWidth: 53 * (cell + gap) + 32 }}>
            {MONTHS.map((m) => (
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 4, minWidth: 53 * (cell + gap) + 32 }}>
            <div
              style={{
                display: 'grid',
